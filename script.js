document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.location.pathname.includes('write_post.html') || window.location.pathname.includes('edit_post.html')) {
        const isEditMode = window.location.pathname.includes('edit_post.html');
        ...
        if (postForm) {
            ...
            const formSubmitButton = postForm.querySelector('.btn-primary');

            if (isEditMode) {
                const params = new URLSearchParams(window.location.search);
                currentPostId = Number(params.get('id'));
                let posts = JSON.parse(localStorage.getItem('posts')) || [];
                let postToEdit = posts.find(p => p.id === currentPostId);

                if (postToEdit) {
                    document.title = `게시글 수정 - ${postToEdit.title}`;
                    document.querySelector('.write-post-container h2').textContent = '게시글 수정';
                    titleInput.value = postToEdit.title;
                    categorySelect.value = postToEdit.category;
                    quill.root.innerHTML = postToEdit.content;
                    formSubmitButton.textContent = '수정 완료';
                } else {
                ...
                document.querySelector('.write-post-container h2').textContent = '새 글 작성';
                formSubmitButton.textContent = '작성 완료';
            }
